...
class MouseJack(object):
    """ Class for scanning, pinging and fingerprinting devices """
    ...
    def add_device(self, address, payload):
        ...
        if address in self.devices:
            ...
            self.devices[address]['address']   = from_display(address)[::-1]
            self.devices[address]['device']    = self.get_hid(payload)
            self.devices[address]['payload']   = payload
    ...
    def scan(self, timeout=5.0, callback=None):
        ...
        while time.time() - start_time < timeout:
            ...
            if len(value) >= 5:
                ...
                self._debug("ch: %02d addr: %s packet: %s" % (self.channels[self.channel_index], to_display(address), to_display(payload)))
                if callback:
                    callback(address, payload)
                else:
                    self.add_device(to_display(address), payload)

        return self.devices

    def sniff(self, timeout, addr_string, callback=None):
        address = from_display(addr_string)[::-1]
        ...
        while time.time() - start_time < timeout:
            ...
            if value[0] == 0:
                ...
                payload = value[1:]
                self._debug("ch: %02d addr: %s packet: %s" % (self.channels[self.channel_index], addr_string, to_display(payload)))
                if callback:
                    callback(address, payload)
                else:
    ...
    def transmit_payload(self, payload):
        self._debug("Sending: " + to_display(payload))
